...
export class NativeMessagingService {
  ...
  init() {
    ipc.platform.onNativeMessaging((message) => this.messageHandler(message));
  }
  ...
  private async messageHandler(msg: LegacyMessageWrapper | Message) {
    ...
    if ("command" in rawMessage && rawMessage.command === "setupEncryption") {
      ...
      if (!userIds.includes(rawMessage.userId)) {
        ipc.platform.sendNativeMessagingReply({
          command: "wrongUserId",
          appId: appId,
        });
        return;
  ...
  private async send(message: any, appId: string) {
    ...
    ipc.platform.sendNativeMessagingReply({ appId: appId, message: encrypted });
  }
